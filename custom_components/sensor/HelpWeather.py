    ...
    if CONF_SUGGESTION in monitored_conditions:
        ...
        if isinstance(SuggestionSensor, dict):
            for variable in SuggestionSensor:
                ...
                if len(sensors) == 0:
                    sensor_Name = SUGGESTION_FORECAST_TYPE['info'][0]
                    measurement = SUGGESTION_FORECAST_TYPE['info'][1]
                    dev.append(HelpWeatherSensor(weatherData, CONF_SUGGESTION, 'info', sensor_Name, isShowWeatherPic, measurement,variable))
